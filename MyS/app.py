...
class User(db.Model):
	...
	gslink=db.Column(db.String(200),nullable=False)
...
class RegisterForm(Form):
	...
	contact= StringField('Contact', [validators.Length(min=6, max=50)])
	affiliation=StringField('Affiliation', [validators.Length(min=2, max=50)])
	gslink=StringField('GSLink', [validators.Length(min=6, max=50)])
...
def register():
	...
	if request.method == 'POST' and form.validate():
		...
		user = User(name = name, email = email, contact= contact, affiliation = affiliation, gslink = gslink, username = username, password = password)
		db.session.add(user)
		#Commit to DB
